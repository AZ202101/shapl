    ...
    if os.path.exists(results_file):
        ...
        for method, feature_order in feature_rankings.items():
            ...
            for i in range(1, len(feature_order) + 1):
                ...
                model.fit(x_train, y_train)
                y_pred = model.predict(x_test)
                new_metric = r2_score(y_test, y_pred) if isinstance(model, lgb.LGBMRegressor) else accuracy_score(
                    y_test, y_pred)
...
def classification_kpi(X, y, S):
    ...
    if len(cols) == 0:
        ...
        try:
            model = lgb.LGBMRegressor(learning_rate=0.3, verbosity=-1, device_type='gpu')
        except Exception:
...
if __name__ == "__main__":
    ...
    try:
        ...
        model = lgb.LGBMRegressor(learning_rate=0.3, verbosity=-1)
    shapley_values, cis_values, qr_cs_values, block_qr_cs_values, random_cs_values, results = benchmark_feature_importance(housing_data_reader, model, filename='housing_benchmark.png')
